...
def implicitEuler_mixedFEM(space_disc: SpaceDiscretisation,
    ...
    VariationalForm = ( 
        inner(u - uold,v) 
        + tau*( 1.0/Re*inner(grad(u), grad(v)) - inner(p, div(v)) + inner(div(u), q) + 1.0/2.0*(inner(dot(grad(u), u), v) - inner(dot(grad(v), u), u)) )
    ...
    for index in tqdm(range(len(time_increments))):
        ...
        try:   
            ...
            logging.exception(e)
            solve(VariationalForm == 0, up, bcs=space_disc.bcs_mixed, nullspace=space_disc.null, solver_parameters=enable_light_monitoring)
